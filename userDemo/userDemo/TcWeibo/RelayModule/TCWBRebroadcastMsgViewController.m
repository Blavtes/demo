...
- (void)viewDidLoad
{
    ...
    // 图片缩略图
    if ([[self.myDict objectForKey:@"image"] isKindOfClass:[UIImage class]]) {
         imageReadyPost = (UIImage*)[self.myDict objectForKey:@"image"];
    }else{
    ...
    if (imageReadyPost) {
        ...
        if ([imageReadyPost isKindOfClass:[UIImage class]]) {
            [imageReadyPost rotateImage:((UIImage*)imageReadyPost).imageOrientation];
        }
        
        if ([imageReadyPost isKindOfClass:[UIImage class]]) {
            [viewImage setAttachedImage:imageReadyPost];
        }else{
            [viewImage setAttachedImage:[UIImage imageWithData:imageReadyPost]];
        }
